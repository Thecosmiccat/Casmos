...
public class TabBarView: View {
    ...
    func replaceTab(_ tab: TabItem, at index:Int) {
        ...
        (self.subviews[index] as? Control)?.backgroundColor = tabFill
        (self.subviews[index].subviews.first as? View)?.backgroundColor = tabFill
    ...
    func redraw() {
        ...
        for i in 0 ..< tabs.count {
            ...
            container.backgroundColor = tabFill
    ...
    override public func setFrameSize(_ newSize: NSSize) {
       ...
       // if previous != newSize.width {
            ...
            for subview in subviews {
                guard !subview.subviews.isEmpty else {
                    continue
                }
                let w = idx == tabs.count - 1 ? defWidth - .borderSize : defWidth
    ...
    public func setSelectedIndex(_ selectedIndex: Int, respondToDelegate: Bool, animated: Bool) {
        if selectedIndex >= self.tabs.count || self.tabs.count == 0 || self.subviews.count <= max(selectedIndex, self.selectedIndex) {
            return
        }
        let deselectItem = self.tabs[self.selectedIndex]
        let deselectView = self.subviews[self.selectedIndex]
        guard let deselectImage = deselectView.subviews.first?.subviews.first else {
            return
        }
        deselectItem.setSelected(false, for: deselectImage, animated: animated)
        self.selectedIndex = selectedIndex
        let selectItem = self.tabs[self.selectedIndex]
        let selectView = self.subviews[self.selectedIndex]
        guard let selectImage = selectView.subviews.first?.subviews.first else {
            return
        }
        selectItem.setSelected(true, for: selectImage, animated: animated)
