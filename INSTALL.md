...
Casmos Settings → General: Keep Original File Names uses the document name in Save and Downloads. Confirm External Links is **on by default** and prompts before opening http(s) URLs. Appearance: Compact Chat List (56pt rows) and Monochrome Folders (gray folder tags). Chat: Double-Click Action (default Reply) and Hide Channel Bottom Buttons (collapses the channel Mute / Discuss bar; header actions stay). Passcode: Lock on Sleep and Hide Content in App Switcher are **on by default**. Experimental: Verbose Logging is **off by default**; turn it on only when you want console and log files. Config: Export / Import a JSON file of `casmos.pref.*` keys (may include a local DeepL key or Cloudflare token).
