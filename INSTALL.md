...
A basic run does not need a DeepL key, the DeepL engine, or translator enabled. Language → Translate Messages stays off. Leave the DeepL field empty (`CASMOS_PLACEHOLDER_DEEPL_KEY` is treated as unset).

## Translator (optional Mac QA)

Language → Translate Messages: enable, cycle Engine (`system` / `extra` / `yandex` / `deepl`), Keep Formatting (default on), Do Not Translate language list, optional Auto-translate Chats, optional DeepL key field. Message context menu Translate uses the selected engine. Poll and todo lists use the local engine when translator is enabled. DeepL without a local key uses the public web endpoint. Keep Formatting sends HTML to Yandex, and to DeepL when a local key is set, so bold / italic / links / code survive. Extra and DeepL-without-key stay plain. DeepL is not required. Keep Deleted Messages (Casmos Settings → Chat, off by default) keeps messages this Mac already downloaded after someone else deletes them.

Casmos Settings → General: Keep Original File Names uses the document name in Save and Downloads. Confirm External Links is **on by default** and prompts before opening http(s) URLs. Hide Phone and Username is **on by default** and removes your number and @username from your own profile, Settings header, and Edit Account values (Username and Change Number stay tappable without blank values). Appearance: Compact Chat List (56pt rows), Monochrome Folders (gray folder tags), and Hide Stories (**on by default**, removes the chat-list Stories strip and avatar story rings). Chat: Double-Click Action (default Reply), Hide Channel Bottom Buttons (collapses the channel Mute / Discuss bar; header actions stay), and Message Filter (comma-separated keywords hide matching incoming text on this Mac; they are not deleted). Passcode: Lock on Sleep and Hide Content in App Switcher are **on by default**. Account passcode / Hide This Account / panic are real (Keychain hashes). Touch ID Reveals Hidden Accounts stays off until biometrics succeed; without Touch ID it shows **NOT WIRED**. Logout on Panic is **off by default**. Experimental: Verbose Logging is **off by default**; turn it on only when you want console and log files. Config: Export / Import a JSON file of `casmos.pref.*` keys (may include a local DeepL key).
...
## Passcode / hide / Touch ID (Mac QA)

1. Add a second account. Casmos Settings → Passcode → Set Account Passcode. Confirm Keychain write: wrong confirm shakes; matching passcodes enable Hide This Account.
2. Turn on Hide This Account. The account disappears from Settings, the tab account menu, and the status-bar account list (current account stays). Unlock Hidden Account with that passcode brings it back this session.
3. Set a Panic Passcode. Type it on the in-app lock overlay (after Privacy → Passcode is set, or Lock on Sleep). Included accounts hide for the session. Logout on Panic stays off unless you turn it on; with it on, those accounts sign out.
4. Touch ID Reveals Hidden Accounts: on a Mac with Touch ID, turning it on prompts LocalAuthentication and stays off if you cancel. After a real Touch ID unlock of the lock overlay, hidden accounts reappear this session. Without biometrics the row shows **NOT WIRED** and is not an on toggle.
5. Cold-start lock (app encryption passcode before launch) only accepts that app passcode — panic / hide are **NOT WIRED** there.
